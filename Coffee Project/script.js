...
function calculateTotal() {
    const basePrices = { "Espresso": 5, "Latte": 8, "Mocha": 10 };
    ...
    if (document.getElementById("syrup").checked) addonsPrice += 2.5;
...
function generateBill() {
    ...
    if (membership === 'gold') {
        membershipDiscount = subtotal * 0.10; // 10% off
    } else if (membership === 'silver') {
        membershipDiscount = subtotal * 0.05; // 5% off
    } else if (membership === 'regular') {
        membershipDiscount = subtotal * 0.03; // 3% off
    }
    ...
    if (quantity >= 5) {  
        ...
        } else if (promoCode === "COFFEE05") {
            discount = subtotal * 0.05; // 5% off
        } else if (promoCode === "VALUE20") {
    ...
    document.getElementById('billMembershipDiscount').textContent = membershipDiscount;
